...
GitHub의 활성 `Protect main` Ruleset은 다음을 강제합니다.
...
1인 개발 흐름을 고려해 승인 리뷰 수는 현재 0명입니다. 협업 인원이 늘어나면 필수 승인 수를 다시 결정합니다.

저장소의 GitHub Actions 정책은 외부 Action을 전체 커밋 SHA로 고정하도록 강제합니다. 워크플로에 Action을 추가하거나 갱신할 때 태그만 사용하지 않습니다.
...
- `README.md`: 플레이어 대상 게임·서비스 소개, 핵심 경험, 캐릭터, 프로토타입 접근 방법
- `docs/MILESTONES.md`: 완료된 마일스톤의 범위와 단계별 설계 결정 기록
- `AGENTS.md`: 구현 제약, 아키텍처 규칙, 에이전트 작업 절차
- `CONTRIBUTING.md`: Git과 협업 규칙
- `.github/pull_request_template.md`: 모든 PR의 완료 체크리스트

README에는 테스트 수, CI 내부 구현, 상세 아키텍처, Git 절차, 구현 변경 이력이나 확정되지 않은 로드맵을 넣지 않습니다. 해당 내용은 책임 문서에서 관리하고 필요한 경우 링크만 제공합니다.
