...
현재 안정 기준선은 **Milestone 3 낙찰 후 처리 및 봉인 시스템 구현 완료** 상태입니다.
...
4. 직접 낙찰한 뒤 처리 전에는 `심판으로`가 비활성인지 확인합니다.
5. 봉인을 하나만 열어 사고 확률과 새 정보를 확인하고 보관합니다.
6. 세 번째 봉인까지 열어 실제 카드명·효과와 `ON_OPEN` 결과가 표시되는지 확인합니다.
7. 판매 대상·가격·공개 단서를 선택해 수락/거절과 소유권 이전을 확인합니다.
8. 소각 비용, `burn_effect`, 인벤토리 제거를 확인합니다.
9. DEBUG를 열어 RNG 사고 판정과 이전 이력이 일반 UI와 분리되는지 확인합니다.
10. 플레이어 사망 또는 10라운드 생존 후 결과 통계와 같은 Seed 재시작을 확인합니다.
11. 창을 1920×1080으로 확대해 Anchor/Container 확장, 텍스트 줄바꿈, 패널 잘림 여부를 다시 확인합니다.
